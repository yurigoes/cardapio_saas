  ...
  if (!mesa) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-slate-950 text-white">
        ...
        <button onClick={() => router.back()} className="text-emerald-400 text-sm">Voltar</button>
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-950 text-white">
      ...
      <header className="sticky top-0 z-10 border-b border-white/5 bg-slate-900/95 backdrop-blur">
        ...
        {mesa.pedido_id && (
          <div className="border-t border-white/5 bg-orange-500/5 px-4 py-2 flex items-center justify-between">
            ...
            <span className={`text-xs font-medium ${
              mesa.pedido_status === "pronto" ? "text-emerald-400" : "text-orange-400"
            }`}>
              {mesa.pedido_status}
            </span>
          </div>
        ...
        {/* feedback de enviado */}
        {enviado && (
          <div className="border-t border-emerald-500/20 bg-emerald-500/10 px-4 py-2 flex items-center gap-2">
            <CheckCircle className="h-4 w-4 text-emerald-400" />
            <p className="text-xs text-emerald-400">Pedido enviado para a cozinha!</p>
          </div>
        )}
        {errMsg && (

...
async function processarPagamentoAvulsa(cobrancaId: string, pag: PagamentoMin) {
  const c = await queryOne<{ id: string; empresa_id: string; status: string }>(
    `SELECT id, empresa_id, status FROM cobrancas_avulsas WHERE id = $1`,
    [cobrancaId]
  );
  if (!c) {
    console.warn(`[MP-SaaS] cobrança avulsa ${cobrancaId} não encontrada`);
    return;
  }
  if (c.status === "paga" && pag.status === "approved") return;
  ...
  if (pag.status === "approved") {
    await query(
      `UPDATE cobrancas_avulsas
          SET status = 'paga',
              ...
              atualizado_em = NOW()
        WHERE id = $1`,
      [c.id, pag.date_approved, String(pag.id)]
    );

    // Tenta desbloquear empresa se não tem mais cobranças vencidas
    await query(
    ...
    console.info(`[MP-SaaS] ✓ cobrança avulsa ${c.id} APROVADA`);
  }
}

...
function fmtEndereco(e: Record<string, unknown>): string {
  const partes = [
...
function renderTemplate(
  ...
  contratada: Record<string, unknown>
): string {
  const enderecoEmp = fmtEndereco(empresa);
  const enderecoCon = fmtEndereco(contratada);
  const hoje = new Date().toLocaleDateString("pt-BR", { day: "2-digit", month: "long", year: "numeric" });

  const map: Record<string, string> = {
    // CONTRATANTE (cliente)
    "contratante_razao_social":  String(empresa.razao_social_full ?? empresa.razao_social ?? empresa.nome_fantasia ?? "—"),
    "contratante_nome_fantasia": String(empresa.nome_fantasia ?? "—"),
    "contratante_cnpj":          String(empresa.cnpj ?? "—"),
    "contratante_endereco":      enderecoEmp,
    "contratante_cidade":        String(empresa.endereco_cidade ?? "—"),
    "contratante_uf":            String(empresa.endereco_uf ?? "—"),
    "contratante_representante": String(empresa.gestor_nome ?? "—"),
    "contratante_cpf":           String(empresa.gestor_cpf ?? "—"),
    "contratante_email":         String(empresa.email ?? "—"),
    "contratante_telefone":      String(empresa.whatsapp ?? empresa.telefone ?? "—"),

    // CONTRATADA (SaaS owner = Three Digital)
    "contratada_razao_social":  String(contratada.razao_social ?? contratada.nome ?? "Three Digital"),
    "contratada_cnpj":          String(contratada.cnpj ?? "—"),
    "contratada_endereco":      enderecoCon,
    "contratada_cidade":        String(contratada.cidade ?? "Salvador"),
    "contratada_email":         String(contratada.email ?? "—"),
    "contratada_telefone":      String(contratada.whatsapp ?? contratada.telefone ?? "—"),

    // Aliases legados (compat com v1)
    "razao_social":   String(empresa.razao_social_full ?? empresa.razao_social ?? empresa.nome_fantasia ?? "—"),
    "nome_fantasia":  String(empresa.nome_fantasia ?? "—"),
    "cnpj":           String(empresa.cnpj ?? "—"),
    "gestor_nome":    String(empresa.gestor_nome ?? "—"),
    "gestor_cpf":     String(empresa.gestor_cpf ?? "—"),
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const rendered = renderTemplate(template.conteudo_html, empresa ?? {}, branding as unknown as Record<string, unknown>);

...
type NavItem  = { href: string; label: string; icon: React.ComponentType<{ className?: string }>; modulo: string | null; suporteOnly?: boolean };
...
const NAV_GROUPS: NavGroup[] = [
  ...
  { titulo: "Financeiro", items: [
    ...
    { href: "/painel/financeiro/mensalidades", label: "Mensalidades", icon: Receipt,   modulo: null },
  ...
  { titulo: "Empresa", items: [
    { href: "/painel/empresa/cadastro", label: "Dados cadastrais", icon: Building2,   modulo: null },
    { href: "/painel/empresa/contrato", label: "Contrato",         icon: FileText,    modulo: null },
...
// Compat: lista plana ainda usada no resto do layout
...
export default function EmpresaLayout({ children }: { children: React.ReactNode }) {
  ...
  function processarItens(items: NavItem[]) {
    return items
      .filter(item => !item.suporteOnly || suporteLiberado)
      .map(item => {
  ...
  const navGroupsRender = NAV_GROUPS
    .map(g => ({ titulo: g.titulo, items: processarItens(g.items) }))
    .filter(g => g.items.length > 0);

  // Compat: lista plana ainda referenciada em alguns checks
  const navItems = navGroupsRender.flatMap(g => g.items);
  ...
  return (
    <div className="flex min-h-screen bg-slate-950 text-white">
      ...
      <aside className="fixed inset-y-0 left-0 flex w-60 flex-col border-r border-white/5 bg-slate-900">
        ...
        {/* Nav agrupado por seções */}
        <nav className="flex-1 overflow-auto py-3">
          {navGroupsRender.map((grupo, gi) => (

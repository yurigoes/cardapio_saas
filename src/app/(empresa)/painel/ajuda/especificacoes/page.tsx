...
const SPECS: Spec[] = [
  ...
  // ─────────────────────────────────────────────────────────────────────
  {
    ...
    itens: [
      ...
      { chave: "Forma 'cartao_caixa'", valor: "Totem mostra tela 'Vá ao caixa' com número GIGANTE + total. Imprime APENAS ticket de identificação (formatarTicketAguardandoPagamento) com código de barras Code 128 inline ([BARCODE128:xxxx] → ESC/POS GS k 73). Cozinha + via final NÃO saem nesse momento. Operador no caixa usa widget no /painel/pedidos: lê código de barras (leitor USB = teclado HID) ou digita nº → POST /api/painel/pedidos/{id}/confirmar-pagamento?numero=X → marca pagamento_confirmado_em → dispara cozinha + via final. Idempotente (chamada repetida não reimprime)." },

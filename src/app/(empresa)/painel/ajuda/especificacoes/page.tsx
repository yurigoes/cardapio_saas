...
const SPECS: Spec[] = [
  ...
  // ─────────────────────────────────────────────────────────────────────
  {
    ...
    itens: [
      ...
      { chave: "Stone (PIX)",   valor: "Driver src/lib/gateways/stone.ts. OAuth client_credentials em api.openbank.stone.com.br/auth/realms/stone_bank/protocol/openid-connect/token. POST /api/v1/pix/payment-links cria QR. Webhook HMAC-SHA256." },
      { chave: "Stone (cartão)", valor: "INTEGRAÇÃO LIMITADA. Stone só oferece Intent intra-Android (totem precisa SER a Stone Smart). Em SaaS com totem tela grande, usamos forma_pagamento='cartao_caixa' — totem mostra 'Vá ao caixa', operador processa manual na maquininha. Pra full integration requer partnership oficial Stone + SDK privado." },
      { chave: "Pagar.me",      valor: "Driver src/lib/gateways/pagarme.ts. Empresa Stone — API REST simples." },
      { chave: "Asaas",         valor: "Driver src/lib/gateways/asaas.ts. PIX/cartão/boleto." },
  ...
  // ─────────────────────────────────────────────────────────────────────
  {
    ...
    itens: [
      ...
      { chave: "Pagamentos no totem", valor: "Dinheiro · PIX (QR) · Cartão pinpad integrado (Cielo TEF/LIO se totem é Android Cielo) · Cartão no caixa (fallback Stone/qualquer adquirente sem integração)" },
      { chave: "Forma 'cartao_caixa' OU 'dinheiro' (no totem)", valor: "Totem mostra tela 'Vá ao caixa' com número GIGANTE + total. Imprime APENAS ticket de identificação (formatarTicketAguardandoPagamento) com código de barras Code 128 inline ([BARCODE128:xxxx] → ESC/POS GS k 73). Cozinha + via final NÃO saem nesse momento. Operador no caixa usa widget no /painel/pedidos: lê código de barras (leitor USB = teclado HID) ou digita nº + escolhe forma real (dinheiro/cartão/pix) → POST /api/painel/pedidos/{id}/confirmar-pagamento?numero=X → marca pagamento_confirmado_em → dispara cozinha + via final. Idempotente (chamada repetida não reimprime). Aplica-se a DINHEIRO+totem e CARTAO_CAIXA — ambos exigem confirmação do caixa antes de liberar cozinha." },
      { chave: "Migration 084", valor: "pedidos.pagamento_confirmado_em TIMESTAMPTZ + índice parcial WHERE pagamento_confirmado_em IS NULL AND forma_pagamento='cartao_caixa' pra dashboard de pendentes." },
      { chave: "Print-agent v1.4", valor: "Suporta marcador [BARCODE128:xxx] no conteúdo do cupom. Converte pra ESC/POS GS k 73 (Code128 Code Set B). Compatível com EPSON/Bematech/Daruma. Impressoras antigas que ignoram GS k: legenda numérica fica visível abaixo." },
      { chave: "Print-agent v1.5 (Windows service)", valor: "install-service.bat agora cria 3 tarefas no Agendador: BOOT (roda no boot como SYSTEM), LOGON (roda no logon do user), WATCHDOG (relança a cada 5min se cair). Detecta node.exe automaticamente e regrava runner.vbs com caminho absoluto (evita falha quando rodando como SYSTEM com PATH minimal). Anti-duplicação: runner verifica tasklist antes de iniciar. Agente fica online mesmo sem ninguém logado." },

...
const SECOES: Secao[] = [
  ...
  },
  {
    ...
    resumo: "Mercado Pago, Cielo (eCommerce/LIO/TEF), Stone, Pagar.me, Asaas, PIX direto, etc. Escolhe um como padrão pra cada método.",
    campos: [
      { nome: "Mercado Pago", descricao: "Access token + chave pública. Suporta Checkout Pro + PreApproval (assinatura)." },
      ...
      { nome: "Cielo TEF", descricao: "Integração Android Intent (POS Cielo no totem que ROD a o app Cielo)." },
      { nome: "Stone PIX", descricao: "Client ID + Client Secret da Stone OpenBank. Apenas PIX e link de pagamento — cartão exige Stone Smart no totem ou pague-no-caixa." },
      { nome: "Pagar.me", descricao: "Empresa Stone — outro gateway, mesma config style." },
      { nome: "Asaas", descricao: "PIX, cartão, boleto. API token simples." },
      { nome: "PIX direto", descricao: "Chave PIX + dados bancários. Gera QR estático, conciliação manual." },
      { nome: "Padrão", descricao: "Gateway usado quando o cliente não escolhe explicitamente (PIX no totem etc)." },
    ],
    fluxos: [
      { titulo: "Configurar Stone PIX", passos: [
        "1× só: cliente cria app em developers.stone.com.br → recebe Client ID + Secret",
        "Stone aprova PIX no plano do cliente (pode levar 24-48h)",
        "Painel: /painel/gateways → '+ Novo gateway' → slug 'stone' → ambiente 'producao'",
        "Cola Client ID + Client Secret + webhook secret (opcional)",
        "Marca como padrão pra PIX se quiser",
        "Testa: totem → faz pedido → escolhe PIX → QR aparece em <2s",
      ]},
    ],
  },
  {
    id: "stone-cartao",
    titulo: "Cartão da Stone (estratégia SaaS)",
    icone: CreditCard,
    rota: "/painel/gateways (config) + totem (uso)",
    resumo: "Stone só permite integração via Android Intent — totem precisa ser a própria maquininha Stone Smart, OU o cliente paga no caixa manualmente. Como o sistema é SaaS e totem é tela grande (não Stone Smart), usamos sempre 'Cartão no caixa'.",
    campos: [
      { nome: "Por que não integra automático?", descricao: "Stone não oferece API LAN/socket pública (apenas TEF Intent intra-Android). Pra integrar full requer parceria comercial + SDK privado." },
      { nome: "Como funciona hoje", descricao: "Totem mostra botão 'Cartão no caixa'. Cliente seleciona, recebe número do pedido GIGANTE na tela. Vai até o caixa, fala o número, atendente processa o cartão na maquininha manualmente." },
      { nome: "PIX da Stone", descricao: "Funciona 100% integrado server-side via Stone OpenBank API. QR gerado direto no totem, cliente paga no app do banco, pagamento confirma via webhook." },
    ...
    fluxos: [
      ...
      { titulo: "Cliente paga PIX Stone no totem (integrado)", passos: [
        ...
        "Cliente abre app do banco, escaneia, confirma",
        "Webhook Stone notifica master → master atualiza status pra 'aprovado'",
        "Cupom é impresso automático",
      ]},
    ],
    dicas: [
      "A Stone tem 3 produtos diferentes (Smart Open, Stone Pos legacy, Stone Marketplace) — cuidado ao buscar docs.",
      "Pra subir pra integração full no futuro, abra ticket no comercial Stone falando que é 'integrador SaaS' — eles têm trilha de partnership.",
      "Por enquanto, 'cartão no caixa' tem UX boa porque o número grande na tela + total grande facilita identificação rápida pelo caixa.",
    ],
  },
  {
    id: "retaguarda",
    ...
    campos: [
      { nome: "Mini-PC mínimo", descricao: "2 vCPU, 4 GB RAM, 20 GB SSD, Linux Ubuntu/Debian. Pode ser Raspberry Pi 4 ou Mini PC NUC." },
      { nome: "Wizard de instalação", descricao: "/admin/retaguardas → '+ Nova retaguarda' gera comando curl com token (24h). Cola no mini-PC e pronto." },
      { nome: "Cloudflare Tunnel", descricao: "1 tunnel por loja na sua conta CF. HTTPS público em loja-X.tthreedigital.com.br sem IP fixo." },
      { nome: "Acesso interno LAN", descricao: "Totens podem usar IP direto: http://192.168.0.50/totem/{slug} sem cert SSL nem DNS." },
      { nome: "Buffer offline", descricao: "Se internet cair, pedidos enfileiram localmente no Redis e drenam quando volta com Idempotency-Key (zero duplicação)." },
      { nome: "Invalidação remota", descricao: "Mudou produto no painel? Master dispara POST /__purge nas retaguardas online — atualiza em <1s sem esperar TTL." },
      { nome: "Métricas no heartbeat", descricao: "Reporter envia a cada 60s: pendentes na fila, enviados, falhados + uso de disco por zona de cache." },
      { nome: "Auto-update", descricao: "Watchtower checa novas imagens a cada 12h e atualiza nginx/redis/cloudflared sozinho." },
      { nome: "Cache em camadas", descricao: "Imagens 7 dias · Cardápio 5 min · Estáticos Next 30 dias · Mutations pass-through." },
      { nome: "Página detalhada", descricao: "/admin/retaguardas/[id] mostra métricas em tempo real + botão purgar cache + desativar." },
    ],
    fluxos: [
      { titulo: "Instalar retaguarda nova (modo wizard, recomendado)", passos: [
        "Master: /admin/retaguardas → botão '+ Nova retaguarda' → escolher slug → copiar comando curl",
        "Mini-PC novo (root): cola o comando, ele puxa config do master via token de 24h",
        "Script pede só CF API Token + Account ID + Zone ID (anotados 1× só por conta CF)",
        "Instala Docker, cria Cloudflare Tunnel, sobe nginx+redis+worker+purger+reporter+cloudflared+watchtower",
        "Em ~3min retaguarda aparece online em /admin/retaguardas",
      ]},
      { titulo: "Instalar retaguarda nova (modo manual)", passos: [
        "Pré-req master (1× só): RETAGUARDA_HEARTBEAT_SECRET no .env + migrations 081/082 + restart",
        "Pré-req CF (1× só): API Token com Tunnel:Edit + DNS:Edit, anotar Account ID + Zone ID",
        "Mini-PC: curl -fsSL .../install.sh | sudo bash — responde tudo interativamente",
      ]},
      { titulo: "Como funciona quando internet cai", passos: [
        "Totem faz POST de pedido → retaguarda nginx tenta master",
        "Master inalcançável → nginx cai pro fallback worker:3001",
        "Worker enfileira no Redis local + gera Idempotency-Key + responde 202",
        "Totem mostra 'Pedido aceito (offline)' — cliente vai embora",
        "Quando internet volta: drainer (a cada 5s) replaya com Idempotency-Key",
        "Master vê chave repetida, dedupe via Redis idem:* — zero duplicação",
      ]},
      { titulo: "Como invalidação remota funciona", passos: [
        "Operador salva produto no painel master",
        "Master: invalidarCardapioPorEmpresa() roda em paralelo:",
        "  1. DEL no Redis local (cache 5min do master)",
        "  2. POST /__purge {slug} pra cada retaguarda online da empresa",
        "Purger sidecar na retaguarda computa md5 da chave + deleta arquivo",
        "Mudança aparece em <1s sem esperar TTL",
      ]},

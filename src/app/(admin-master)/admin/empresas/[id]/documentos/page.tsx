...
export default function DocsEmpresaPage() {
  ...
  async function validar(doc: Doc, validado: boolean) {
    setBusy(doc.id);
    try {
      const obs = !validado
      ...
      const r = await fetch(`/api/admin/empresas/${id}/documentos`, {
        ...
        body: JSON.stringify({ doc_id: doc.id, validado, observacao: obs }),
      });
            ...
            return (
              <div
                ...
                className={`rounded-xl border p-4 ${
                  d.validado ? "border-emerald-500/30 bg-emerald-500/5" : "border-white/10 bg-white/5"
                }`}
              >
                ...
                {d.observacao && (
                  <p className="mb-2 rounded bg-amber-500/10 px-2 py-1 text-xs text-amber-300">
                    Obs: {d.observacao}
                  </p>
                )}
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => validar(d, !d.validado)}
                    disabled={busy === d.id}
                    className={`flex-1 flex items-center justify-center gap-1 rounded-lg px-3 py-1.5 text-xs font-medium ${
                      d.validado
                  ...
                  >
                    {busy === d.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> :
                      d.validado ? <><X className="h-3.5 w-3.5" /> Invalidar</>

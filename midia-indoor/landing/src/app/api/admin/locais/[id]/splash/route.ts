/**
 * POST /api/admin/locais/[id]/splash — define o splash do local (Default Layout).
 * Aceita 1+ arquivos (imagens/vídeos).
 */
import { NextRequest, NextResponse } from "next/server";
import { exigirMaster } from "@/lib/admin-auth";
import { definirSplashLocal } from "@/lib/conteudo";
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const r = await definirSplashLocal(params.id, files.map(f => ({ arquivo: f, nomeArquivo: f.name })));
    if (!r.ok) return NextResponse.json({ ok: false, error: r.erro }, { status: 400 });
    return NextResponse.json({ ok: true, telas_atualizadas: r.telas_atualizadas });
  } catch (err) {
    console.error("[locais/splash]", err);

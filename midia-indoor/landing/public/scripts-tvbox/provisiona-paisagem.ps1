...
# Suporta provisionamento seletivo via -Componentes. Sem o parametro, abre prompt.
#
# Uso:
#   .\provisiona-paisagem.ps1 -Ip 192.168.15.51                          # prompt interativo
#   .\provisiona-paisagem.ps1 -Ip 192.168.15.51 -Componentes "tudo"      # tudo do zero
#   .\provisiona-paisagem.ps1 -Ip 192.168.15.51 -Componentes "launcher"  # so launcher
#   .\provisiona-paisagem.ps1 -Ip 192.168.15.51 -Componentes "xibo,rustdesk"
...
# 4. Boot animation + rotacao (so se flags.Boot)
if ($flags.Boot) {
  $videoOk = $false
...
# 5. RustDesk (so se flags.RustDesk)
$rdId = ""
$monitor = $null
if ($flags.RustDesk) {
